...
export default function DeliveryPage() {
  ...
  const [orders, setOrders] = useState(
    adminOrders.filter((o) => ['confirmed', 'preparing', 'delivering', 'delivered'].includes(o.status))
  )
  ...
  return (
    <div
      ...
      style={{ background: '#0A0A0A', direction: 'rtl', fontFamily: 'Cairo, sans-serif' }}
    >
      {/* Top bar */}
      <div
      ...
      >
        ...
        >
          ...
          <span className="font-black text-white text-sm hidden sm:block">أبو دغش</span>
        </button>
      ...
      <div className="pt-20 pb-24 px-4 max-w-2xl mx-auto">
        ...
        <div
          ...
          style={{ background: 'linear-gradient(135deg, #1A0000, #3D0000)', border: '1px solid rgba(204,0,0,0.2)' }}
        >
          <div
          ...
          <div
            ...
            style={{ background: 'rgba(204,0,0,0.3)', border: '1px solid rgba(204,0,0,0.4)' }}
          >
          ...
          <div className="relative z-10">
            ...
            <p className="text-brand-red text-xs font-semibold">
              {lang === 'ar' ? `${activeOrders.length} توصيلات معلقة` : `${activeOrders.length} משלוחים ממתינים`}
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-3 mb-6">
          <StatCard icon={Package} label={lang === 'ar' ? 'إجمالي' : 'סה"כ'} value={orders.length} color="#3B82F6" />
          <StatCard icon={Truck} label={lang === 'ar' ? 'نشط' : 'פעיל'} value={activeOrders.length} color="#F97316" />
          <StatCard icon={CheckCircle} label={lang === 'ar' ? 'مكتمل' : 'הושלם'} value={completedOrders.length} color="#16A34A" />
        </div>
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-white/50 text-xs">{lang === 'ar' ? 'أرباح اليوم (تقديري)' : 'רווחי היום (משוער)'}</p>
              <p className="text-yellow-400 font-black text-lg">₪{todayEarnings}</p>
            </div>
          </div>
        ...
        >
          {[
            { id: 'active', label_ar: `نشط (${activeOrders.length})`, label_he: `פעיל (${activeOrders.length})`, color: '#F97316' },
            { id: 'completed', label_ar: `مكتمل (${completedOrders.length})`, label_he: `הושלם (${completedOrders.length})`, color: '#16A34A' },
          ].map((tab) => (
            <button
              key={tab.id}
        ...
        <div className="space-y-3">
          {displayOrders.length === 0 ? (
            <div className="text-center py-16">
              <CheckCircle size={48} className="text-green-500 mx-auto mb-3 opacity-60" />
              <p className="text-white/50 font-semibold">
                {lang === 'ar' ? 'لا توجد توصيلات في هذا القسم' : 'אין משלוחים בקטגוריה זו'}
              </p>
            </div>
          ) : (
              ...
              >
                ...
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <div className="text-left">
                    <p className="text-brand-red font-black text-base">₪{order.total}</p>
                    <p className="text-white/40 text-xs">{order.items} منتجات</p>
                  </div>
                </div>
